...
def get_args():
    ...
    parser.add_argument('-a', '--ascending', action='store_true', default=False,
                        help='Print and plot stats in ascending order by frequency.')
    parser.add_argument('-d', '--descending', action='store_true', default=False,
                        help='Print and plot stats in descending order by frequency.')
    parser.add_argument('-g', '--generate', action='store_true', default=False,
                        help='Skip plotting & prompt user for generating numbers in SD.')
    parser.add_argument('--nosave', action='store_true', default=False, help='Don\'t save a raw numbers file.')
    parser.add_argument('-b', '--bins', nargs=1, default=[50],
                        help='Number of bins for plotting histograms. Default: 50')
    parser.add_argument('--stdev', nargs=1, default=[1],
                        help='+/- Stdev. to use for figure 5 and option -c. Default: 1')
    parser.add_argument('--savefile', action='store_true', default=False,
                        help='Save the calottery .txt file as fantasy5results.txt. Default: False')
    parser.add_argument('--uselocal', action='store_true', default=False,
                        help='Use local fantasy5results.txt file from --savefile, instead of fetch. Default: False')
...
def print_stats_list(label, l, mean_list, mode_list_rounded, stdev_list, list_rounded):
    print("{:<36}: {:.3f}".format(label + " Max", l[-1]))
    print("{:<36}: {:.3f}".format(label + " Min.", l[0]))
    print("{:<36}: {:.3f}".format(label + " Mean", mean_list))
    print("{:<36}: {:.3f}".format(label + " Median", median(l)))
    print("{:<36}: {:.3f}".format(label + " Mode", my_mode(l)))
...
def plot_histogram(title, xlabel, factor, list, mean_list, stdev_list, mode_list_rounded, current_num, current_num_in):
    ...
    plots[1].axvline(median(list), label="Median: {:.3f}".format(median(list)), linestyle="--", color="#00FF00",
                     linewidth=0.9)
    plots[1].axvline(mode_list_rounded, label="Rounded Mode: {}".format(mode_list_rounded), linestyle="--",
                     color="#FF8C00", linewidth=0.9)
    plots[1].axvline(current_num_in, label="Last Winning {}: {:.3f}".format(factor, current_num_in), linestyle="-.",
                     color="#9932CC", linewidth=1)
...
def print_stats(histogram_items, histogram_dict, ascend_hist, tot_cnt, tot_sum, daily_sums, daily_means,
    ...
    if GENERATE_NUMS:
        ...
        while True:
            ...
            while True:
                ...
                if not (mean_daily_stdevs + sd > stdev_nums > mean_daily_stdevs - sd):
                    continue
                sd = stdev_daily_means * F5_STDEV
                if not (mean_daily_means + sd > mean_nums > mean_daily_means - sd):
                    continue
                sd = stdev_daily_cnt_mean * F5_STDEV
                if not (mean_daily_cnt_mean + sd > mean_freq_nums > mean_daily_cnt_mean - sd):
                    continue
                sd = stdev_daily_cnt_stdev * F5_STDEV
                if not (mean_daily_cnt_stdev + sd > stdev_freq_nums > mean_daily_cnt_stdev - sd):
                    continue
    ...
    for line in all_lines:
        ...
        sd = stdev_daily_stdevs * F5_STDEV
        t_1 = int(mean_daily_stdevs + sd > stdev_line > mean_daily_stdevs - sd)
        sd = stdev_daily_means * F5_STDEV
        t_2 = int(mean_daily_means + sd > mean_line > mean_daily_means - sd)
        sd = stdev_daily_cnt_mean * F5_STDEV
        t_3 = int(mean_daily_cnt_mean + sd > mean_freq_line > mean_daily_cnt_mean - sd)
        sd = stdev_daily_cnt_stdev * F5_STDEV
        t_4 = int(mean_daily_cnt_stdev + sd > stdev_freq_line > mean_daily_cnt_stdev - sd)

        in_stdev[t_1 | (t_2 << 1) | (t_3 << 2) | (t_4 << 3)] += 1
    ...
    plt.bar(["0", "1", "2", "1,2", "3", "1,3", "2,3", "1,2,3", "4", "1,4", "2,4", "1,2,4", "3,4", "1,3,4", "2,3,4",
             "1,2,3,4"],
...
def main():
    ...
    if ARGS.ascending or ARGS.descending:
